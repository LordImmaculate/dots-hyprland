...
# MISC (For .config/* but not AGS, not ZSH, not Hyprland)
case $SKIP_MISCCONF in
  true) sleep 0;;
  *)
    for i in $(find .config/ -mindepth 1 -maxdepth 1 ! -name 'ags' ! -name '.zshrc' ! -name 'hypr' -exec basename {} \;); do
#      i=".config/$i"
...
case $SKIP_ZSH in
  true) sleep 0;;
  *)
    v rsync -av --delete .config/.zshrc "$HOME"/.zshrc
    ;;
esac
